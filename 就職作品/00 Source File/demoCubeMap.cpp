...
void CDemoCubeMap::Init()
{
	...
	//テクスチャ読み込み
	SetVisualDirectory();
	D3DX11CreateShaderResourceViewFromFile(RENDERER::GetDevice(), "Sky//SunlessCirruscover4k_f.dds", NULL,NULL, &m_pTexture_f, NULL);//Front
	assert(m_pTexture_f);
	D3DX11CreateShaderResourceViewFromFile(RENDERER::GetDevice(), "Sky//SunlessCirruscover4k_b.dds", NULL,NULL, &m_pTexture_b, NULL);//Back
	assert(m_pTexture_b);
	D3DX11CreateShaderResourceViewFromFile(RENDERER::GetDevice(), "Sky//SunlessCirruscover4k_u.dds", NULL,NULL, &m_pTexture_u, NULL);//Up
	assert(m_pTexture_u);
	D3DX11CreateShaderResourceViewFromFile(RENDERER::GetDevice(), "Sky//SunlessCirruscover4k_d.dds", NULL,NULL, &m_pTexture_d, NULL);//Down
	assert(m_pTexture_d);
	D3DX11CreateShaderResourceViewFromFile(RENDERER::GetDevice(), "Sky//SunlessCirruscover4k_r.dds", NULL,NULL, &m_pTexture_r, NULL);//Right
	assert(m_pTexture_r);
	D3DX11CreateShaderResourceViewFromFile(RENDERER::GetDevice(), "Sky//SunlessCirruscover4k_l.dds", NULL,NULL, &m_pTexture_l, NULL);//Left
	assert(m_pTexture_l);
...
void CDemoCubeMap::Update()
{
}

void CDemoCubeMap::Draw()
{
